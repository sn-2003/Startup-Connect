...
export default function AiMentorChat() {
  ...
  return (
    <>
      ...
      {!open && (
        <button
          className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 bg-blue-600 hover:bg-blue-700 text-white rounded-full p-3 sm:p-4 shadow-lg flex items-center justify-center hover:scale-110 transition-transform duration-200 group"
          onClick={() => setOpen(true)}
          aria-label="Open AI Mentor Chat"
        >
          <span className="mr-1 sm:mr-2 text-lg sm:text-xl animate-waving-hand">👋</span>
          <MessageCircle className="w-5 h-5 sm:w-6 sm:h-6" />
        </button>
      )}
      {/* Chat Window */}
      {open && (
        <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 w-[90vw] sm:w-96 max-w-full max-h-[85vh] sm:max-h-[80vh] bg-white rounded-2xl sm:rounded-3xl shadow-2xl border border-blue-100 flex flex-col overflow-hidden animate-fade-in">
          <div className="flex items-center justify-between bg-blue-600 text-white px-4 sm:px-6 py-3 sm:py-4">
            <div className="flex items-center space-x-2 sm:space-x-3">
              <span className="text-xl sm:text-2xl">🤖</span>
              <div>
                <span className="font-extrabold text-base sm:text-lg">Nova</span>
