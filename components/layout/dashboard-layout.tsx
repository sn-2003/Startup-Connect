...
export default function DashboardLayout({ children, activeTab, onTabChange }: DashboardLayoutProps) {
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row bg-gradient-to-br from-indigo-50 via-blue-50 to-gray-100">
      ...
      `}>
        ...
        <div className="px-4 sm:px-8 py-4 sm:py-5 border-b border-gray-200">
          <div className="flex items-center space-x-2">
            ...
            />
            <span className="text-lg sm:text-xl font-bold text-gray-900">StartupGram</span>
          </div>
        </div>

        {/* Navigation */}
        <nav className="flex-1 px-3 sm:px-4 py-4 sm:py-6 space-y-4 sm:space-y-6 overflow-y-auto">
          {/* Main Navigation */}
          <div>
            <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 sm:mb-3 px-1">MAIN</h3>
                ...
                return (
                  <li key={item.id}>
                    <button
                      ...
                      className={`
                        w-full flex items-center space-x-3 px-2 sm:px-3 py-3 sm:py-2 rounded-lg text-left transition-all duration-200 group relative min-h-[44px] sm:min-h-[auto]
                        ${activeTab === item.id
          ...
          {/* Personal Navigation */}
          <div>
            <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 sm:mb-3 px-1">PERSONAL</h3>
            <ul className="space-y-1">
        ...
        <div className="p-3 sm:p-4 border-t border-gray-200">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className="w-full justify-start text-gray-700 hover:bg-gray-100 min-h-[44px] sm:min-h-[auto] px-2 sm:px-3">
      ...
      {/* Main content */}
      <div className="flex-1 lg:ml-0 min-h-screen flex flex-col w-full">
        {/* Header */}
        <header className="bg-slate-50/80 backdrop-blur-lg border-b border-slate-100 px-3 sm:px-4 py-3 sm:py-4 shadow-md transition-all duration-200 flex items-center justify-between sticky top-0 z-30">
          <div className="flex items-center space-x-2 sm:space-x-4">
            <Button
              variant="ghost"
              size="sm"
              className="lg:hidden h-8 w-8 sm:h-9 sm:w-9 p-1.5 sm:p-2"
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="sm" className="flex items-center space-x-1 sm:space-x-2 h-8 sm:h-9 px-1.5 sm:px-2">
