...
import { CoinDisplay, CoinChange } from '@/components/ui/coin-display';
import { Coins, History } from 'lucide-react';

export default function UserSettings() {
  const { user, logout } = useAuth();
  const router = useRouter();
  const { balance: coinBalance, transactions, loading: coinsLoading, refreshCoins } = useCoins();
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center space-x-2">
            ...
            <span>My Coins</span>
            <CoinDisplay balance={coinBalance} size="md" animated />
          </CardTitle>
          <CardDescription>
            Earn coins by completing actions on the platform. Coins will be redeemable for rewards in the future.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          {/* Coin Balance */}
          <div className="bg-gradient-to-r from-yellow-50 to-orange-50 border border-yellow-200 rounded-lg p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-yellow-600">Keep earning to unlock future rewards!</p>
              </div>
              <div className="text-right">
                <div className="text-3xl font-bold text-yellow-800">{coinBalance.toLocaleString()}</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-3">
                <div className="flex items-center justify-between p-3 bg-blue-50 rounded-lg">
                  <span className="text-sm text-blue-800">Create a startup</span>
                  <Badge className="bg-blue-100 text-blue-800">+100</Badge>
                </div>
                <div className="flex items-center justify-between p-3 bg-green-50 rounded-lg">
                  <span className="text-sm text-green-800">Post a job</span>
                  <Badge className="bg-green-100 text-green-800">+75</Badge>
                </div>
                <div className="flex items-center justify-between p-3 bg-purple-50 rounded-lg">
                  <span className="text-sm text-purple-800">Upload resume</span>
                  <Badge className="bg-purple-100 text-purple-800">+30</Badge>
                </div>
                <div className="flex items-center justify-between p-3 bg-orange-50 rounded-lg">
                  <span className="text-sm text-orange-800">Apply to job</span>
                  <Badge className="bg-orange-100 text-orange-800">+25</Badge>
                </div>
              </div>
              <div className="space-y-3">
                <div className="flex items-center justify-between p-3 bg-pink-50 rounded-lg">
                  <span className="text-sm text-pink-800">Review a tool</span>
                  <Badge className="bg-pink-100 text-pink-800">+20</Badge>
                </div>
                <div className="flex items-center justify-between p-3 bg-indigo-50 rounded-lg">
                  <span className="text-sm text-indigo-800">Give startup feedback</span>
                  <Badge className="bg-indigo-100 text-indigo-800">+15</Badge>
                </div>
                <div className="flex items-center justify-between p-3 bg-teal-50 rounded-lg">
                  <span className="text-sm text-teal-800">Daily login</span>
                  <Badge className="bg-teal-100 text-teal-800">+10</Badge>
                </div>
                <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <span className="text-sm text-gray-800">Vote on startup</span>
