...
export default function ResumeModule() {
  ...
  const handleProfileSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // Always try to update first, if it fails then create
      let response = await apiClient.updateResume(resumeData);
      
      if (!response.success && response.error?.includes('not found')) {
        // Resume doesn't exist, create it
        response = await apiClient.createResume(resumeData);
      }
      
      if (response.success && response.data) {
        setResume(response.data);
        setSkills(response.data.skills || []);
        alert(resume ? 'Resume updated successfully!' : 'Resume created successfully!');
      } else {
        alert('Error saving resume: ' + (response.error || 'Unknown error'));
      }
    } catch (error) {
  ...
  const addExperience = () => {
    const newExperience: Experience = {
      ...
      current: false,
      description: '',
      order: resume?.experience ? resume.experience.length : 0,
      resumeId: resume?.id || '',
      createdAt: new Date(),
      updatedAt: new Date(),
    };
  ...
  const updateExperience = (id: string, updates: Partial<Experience>) => {
    ...
    const updatedExperience = (resume.experience || []).map((exp: Experience) =>
  ...
  const removeExperience = (id: string) => {
    ...
    const updatedExperience = (resume.experience || []).filter((exp: Experience) => exp.id !== id);
    setResume({ ...resume, experience: updatedExperience });
  };
  ...
  const addEducation = () => {
    const newEducation: Education = {
      ...
      endDate: '',
      gpa: '',
      order: resume?.education ? resume.education.length : 0,
      resumeId: resume?.id || '',
      createdAt: new Date(),
      updatedAt: new Date(),
    };
  ...
  const updateEducation = (id: string, updates: Partial<Education>) => {
    ...
    const updatedEducation = (resume.education || []).map((edu: Education) =>
  ...
  const removeEducation = (id: string) => {
    ...
    const updatedEducation = (resume.education || []).filter((edu: Education) => edu.id !== id);
    setResume({ ...resume, education: updatedEducation });
  };
  ...
  const addCustomSection = () => {
    const newSection: CustomSection = {
      ...
      title: '',
      content: '',
      order: (resume?.customSections ? resume.customSections.length : 0) + 1,
      resumeId: resume?.id || '',
      createdAt: new Date(),
      updatedAt: new Date(),
    };
  ...
  const updateCustomSection = (id: string, updates: Partial<CustomSection>) => {
    ...
    const updatedSections = (resume.customSections || []).map((section: CustomSection) =>
  ...
  const removeCustomSection = (id: string) => {
    ...
    const updatedSections = (resume.customSections || []).filter((section: CustomSection) => section.id !== id);
    setResume({ ...resume, customSections: updatedSections });
  };
  ...
  const moveCustomSection = (id: string, direction: 'up' | 'down') => {
    ...
    // Update order values
    sections.forEach((section: CustomSection, idx: number) => {
      section.order = idx + 1;
    });
              ...
              {!resume?.customSections || resume.customSections.length === 0 ? (
                ...
                <div className="space-y-6">
                  {resume.customSections
                    .sort((a: CustomSection, b: CustomSection) => a.order - b.order)
                    .map((section: CustomSection, index: number) => (
            ...
            <CardContent>
              {resume ? (
                <div className="max-w-4xl mx-auto bg-white border rounded-lg p-8 space-y-6">
                  ...
                  {/* Custom Sections */}
                  {resume.customSections && resume.customSections
                    .sort((a: CustomSection, b: CustomSection) => a.order - b.order)
                    .filter((section: CustomSection) => section.title && section.content)
