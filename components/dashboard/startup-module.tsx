  ...
  return (
    <div className="space-y-6">
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="min-w-0">
                        <h3 className="text-sm font-medium text-gray-900 truncate">{startup.name}</h3>
                        <p className="text-xs text-gray-500 truncate">{startup.industry}</p>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <Card>
                ...
                <CardContent>
                  {jobs.length === 0 ? (
                    ...
                    <div className="space-y-4">
                      {jobs.map((job) => (
                        <div key={job.id} className="border rounded-lg p-4 hover:bg-gray-50 transition-colors overflow-hidden">
                          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                            <div className="flex-1 min-w-0">
                              <div className="flex items-start space-x-3 mb-2">
                                <Avatar className="h-10 w-10 flex-shrink-0">
                                  <AvatarImage src={job.startup?.logo || undefined} alt={job.startup?.name || ''} />
                                  <AvatarFallback>
                                    <Building2 className="h-5 w-5" />
                                  </AvatarFallback>
                                </Avatar>
                                <div className="min-w-0">
                                  ...
                                  <div className="flex items-center space-x-2 text-sm text-gray-600">
                                    <span className="truncate">{job.startup?.name}</span>
                            ...
                            <div className="flex flex-wrap gap-2 justify-end">
                              ...
                              >
                                ...
                                <span className="sr-only sm:not-sr-only">Applicants</span>
                              </Button>
                              ...
                              {!job.listed ? (
                                <>
                                  ...
                                  >
                                    <span className="sm:mr-1.5">Relist</span>
                                  </Button>
                                  <Badge variant="destructive" className="text-xs flex items-center h-8 px-2.5">
                                    Delisted
                                  </Badge>
                                </>
                              ) : (
                                ...
                                >
                                  <span className="sm:mr-1.5">Delist</span>
                                </Button>
                              )}
                            </div>
                          </div>
      ...
      <Dialog open={!!previewJob} onOpenChange={open => { if (!open) setPreviewJob(null); }}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto p-0 sm:p-6 w-full">
            ...
            <DialogHeader className="px-4 sm:px-0 pt-4 sm:pt-0">
              <DialogTitle className="flex flex-col sm:flex-row items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-2">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-lg font-semibold">{previewJob?.title}</span>
                </div>
              </DialogTitle>
              <DialogDescription className="px-1 sm:px-0">
            ...
            {previewJob && (
              <div className="space-y-4 p-4 sm:p-0">
                ...
                <div>
                  ...
                  <p className="text-gray-600 whitespace-pre-line break-words">{previewJob.description}</p>
                </div>
                    ...
                    <ul className="space-y-2">
                      {previewJob.requirements.map((req, idx) => (
                        <li key={idx} className="flex items-start space-x-2">
                          ...
                          <span className="text-gray-600 break-words">{req}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
