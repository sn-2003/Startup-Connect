...
import { Job, Application, CustomAnswer } from '@/lib/types';
...
export default function JobBoard() {
  ...
  const handleJobSelect = (job: Job, showApplication = false) => {
    setSelectedJob(job);
  ...
  const handleApply = async (jobId: string) => {
    ...
    setApplying(jobId);
    try {
      const application = await mockAPI.applyToJob(user.id, jobId, customAnswers);
  ...
  const handleSaveJob = async (jobId: string) => {
    ...
    try {
      ...
      if (isCurrentlySaved) {
        ...
        await mockAPI.saveJob(user.id, jobId);
  ...
  const canApply = (job: Job) => {
    if (!job.customQuestions.length) return true;
                      ...
                      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                        ...
                        {selectedJob && (
                          <div className="space-y-6">
                            ...
                            {selectedJob.customQuestions.length > 0 && showApplicationForm && (
