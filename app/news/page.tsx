...
interface ArticleCardProps {
  ...
  onView: (newsId: string, url: string) => void;
  saving: string | null;
  viewed: boolean;
...
const ArticleCard: React.FC<ArticleCardProps> = ({
  ...
  const getCategoryColor = (category: string) => {
    ...
    return colors[item.category as keyof typeof colors] || colors.GENERAL;
  };
  ...
  return (
    <article 
      className={`bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 ${
        listView ? 'flex' : ''
      } ${recentlyViewed ? 'pulse-viewed' : ''}`}
    >
      <div className={`relative ${listView ? 'w-80 flex-shrink-0' : ''}`}>
        <div 
          className={`${listView ? 'h-full' : 'h-48'} cursor-pointer`}
          onClick={() => onView(item.id, item.url)}
        >
          {item.image ? (
            <Image
              src={item.image}
              ...
              sizes={listView ? "(max-width: 768px) 100vw, 320px" : "(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"}
            />
          ) : (
      ...
      <div className={`p-6 flex-1 ${listView ? 'flex flex-col justify-between' : ''}`}>
        <div>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex-1">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent shadow-sm text-sm sm:text-base"
                />
              </div>
            </div>
            ...
            <div className="flex gap-2 sm:gap-3">
              <select
                ...
                className="px-3 sm:px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent shadow-sm text-sm sm:text-base"
              >
              ...
              <div className="flex border border-gray-300 rounded-xl overflow-hidden shadow-sm">
                <button
                  ...
                  className={`px-3 sm:px-4 py-3 transition-colors text-sm sm:text-base ${
                    viewMode === 'grid' 
              ...
              {user && (
                <button
                  ...
                  className={`flex items-center space-x-1 sm:space-x-2 px-3 sm:px-4 py-3 rounded-xl text-xs sm:text-sm font-medium transition-all duration-200 shadow-sm ${
                    showSavedOnly
                      ? 'bg-blue-500 text-white shadow-lg'
                      : 'bg-white text-gray-600 hover:bg-gray-50 border border-gray-200 hover:shadow-md'
                  }`}
                  aria-label={showSavedOnly ? 'Show all news' : 'Show saved only'}
                >
                ...
                {categories.map((category) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex-shrink-0 flex items-center space-x-1.5 px-3 py-2 rounded-xl text-xs font-medium transition-all duration-200 whitespace-nowrap ${
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {featuredNews.map((item, index) => (
                <ArticleCard 
                  key={item.id} 
                  item={item} 
                  isSaved={isNewsSaved(item.id)}
                  onSave={handleBookmark}
                  onView={handleViewArticle}
                  saving={saving}
                  viewed={viewedArticles.has(item.id)}
                  recentlyViewed={recentlyViewed.has(item.id)}
                />
              ))}
            </div>
          </div>
