...
const latestNews = [
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setCurrentInsight((prev) => (prev + 1) % founderInsights.length);
    }, 5000);
  ...
  // If user is logged in, show the insights, tools, and news page
  if (user) {
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
            <Card className="bg-gradient-to-r from-purple-50 to-blue-50 border-purple-200 overflow-hidden">
                  ...
                  <div className="relative z-10 w-full px-8">
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <blockquote className="text-lg font-medium text-gray-900 mb-3 italic">
                          "{founderInsights[currentInsight].quote}"
                        </blockquote>
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="font-semibold text-gray-900">{founderInsights[currentInsight].author}</p>
                            <p className="text-sm text-gray-600">{founderInsights[currentInsight].role}</p>
                          </div>
                          <Badge variant="secondary" className="bg-purple-100 text-purple-700">
                            {founderInsights[currentInsight].company}
                          </Badge>
                        </div>
                      </div>
                    </motion.div>
                    ...
                    {founderInsights.map((_, index) => (
                      <button
                        ...
                        className={`w-2 h-2 rounded-full transition-all ${
                          index === currentInsight ? "bg-purple-600 w-6" : "bg-purple-300"
                        }`}
          ...
          {/* Featured AI Tools - Two at a time */}
          <motion.div
          ...
          >
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-gray-900">Featured AI Tools for Startups</h2>
              ...
              <CardContent className="p-0">
                <div className="relative h-64">
                  ...
                  <div className="relative z-10 w-full h-full flex items-center px-8">
                    ...
                    >
                      {featuredTools.slice(currentToolPair * 2, currentToolPair * 2 + 2).map((tool, index) => (
                        <div key={index} className="bg-white/80 backdrop-blur-sm rounded-lg p-6 shadow-lg">
                          <div className="flex items-start space-x-4">
                            <div className="w-16 h-16 rounded-lg bg-gray-200 flex items-center justify-center border">
                              <span className="text-gray-600 text-xs">{tool.name}</span>
                            </div>
                            <div className="flex-1">
                              ...
                              <Button size="sm" className="w-full mt-3" onClick={() => window.open(tool.url, "_blank")}>
                                Try Now
                  ...
                  <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-2">
                    {Array.from({ length: Math.ceil(featuredTools.length / 2) }).map((_, index) => (
                      <button
                        key={index}
                        className={`w-2 h-2 rounded-full transition-all ${
                          index === currentToolPair ? "bg-blue-600 w-6" : "bg-blue-300"
                        }`}
                ...
                onClick={() => {
                  if (typeof window !== 'undefined') {
                    ...
                    if (chatBtn) {
                      ...
                      // Fallback: redirect to main page or login
