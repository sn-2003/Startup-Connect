...
export async function POST(
  ...
  try {
    ...
    if (action === 'rate') {
      ...
      // Award coins for rating/reviewing tool
      if (review && review.trim()) {
        await CoinSystem.awardCoins(user.id, 'TOOL_REVIEW', { toolId: params.id });
      } else {
        await CoinSystem.awardCoins(user.id, 'TOOL_RATE', { toolId: params.id });
      }
